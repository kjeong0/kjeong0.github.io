$(document).ready(function () {
	$(window).scroll(function (evt) {
		var opacity = 1- $(window).scrollTop()/($('#overlay-white').height()*3);
		hundredEverything();
		if (opacity >= 0) {
		...
		} else if ($(window).scrollTop() > $('#overlay-white').height()*5){
			var goUp = ($(window).scrollTop() - ($('#overlay-white').height()*5)) /2;
